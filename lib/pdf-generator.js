...
async function gerarPDF({ cliente, captura, veiculo, fotoBuffer, historico, cameraNome }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // =========================================
      // CORPO DE TEXTO CUSTOM (opcional, por cliente)
      // =========================================
      // Renderiza apenas se cliente.pdf_corpo_texto estiver preenchido.
      // Quando ativo: pág 1 = advertência textual, pág 2 = evidência.
      // Clientes sem o campo mantêm o layout original (1 página).
      if (cliente.pdf_corpo_texto && cliente.pdf_corpo_texto.trim()) {
        ...
        drawLine(doc);
        doc.moveDown(0.4);
        doc.fontSize(9).font(FONT_REGULAR).fillColor('#222222').text(
          corpoTexto,
          40,
          doc.y,
          {
        ...
        // Evidência sempre em página nova com mini-cabeçalho.
        doc.addPage();
        doc.fontSize(12).font(FONT_BOLD).fillColor('#000000')
          .text(`EVIDÊNCIA DA OCORRÊNCIA — PLACA ${captura.placa}`, { align: 'center' });
        doc.moveDown(0.2);
        doc.fontSize(9).font(FONT_REGULAR).fillColor('#555555')
          .text(
            `Radar ${cameraNome || '—'}  ·  ${dataStr} ${horaStr}  ·  ${velReport} km/h`,
            { align: 'center' }
          );
        doc.moveDown(0.4);
      }
      ...
      // =========================================
      // GRID DE HISTÓRICO (10 por linha)
      // =========================================
