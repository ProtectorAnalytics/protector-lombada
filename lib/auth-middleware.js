...
const { checkAdminRateLimit } = require('./rate-limiter');
 ...
 * Inclui rate limiting por IP (60 req/min).
 *
...
async function autenticar(req, rolesPermitidos = ['super_admin']) {
  ...
  if (!rateCheck.allowed) {
    throw { status: 429, error: 'Muitas requisições. Tente novamente em breve.' };
  }
