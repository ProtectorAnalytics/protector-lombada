...
/**
 * Converte o vehicleId do AlarmInfoPlate em número, ou null quando ausente.
 *
 * Não usar `Number.isFinite(+v)` direto: `+null` e `+''` valem 0 e passam no
 * teste. Um vehicle_id 0 gravado em toda captura de uma câmera que não envia o
 * campo faria o dedupe de api/captura.js tratar passagens distintas como
 * retransmissão e descartá-las. Zero é rejeitado por ser sentinela, não id.
 */
function parseVehicleId(v) {
...
/**
 * Escapa entidades HTML para interpolação segura em templates de e-mail.
 * Diferente de sanitize(): preserva o conteúdo original (não remove caracteres),
 * apenas neutraliza a interpretação como HTML — evita injeção de HTML/links em
 * e-mails que interpolam dados não confiáveis (formulários públicos, OCR).
 */
